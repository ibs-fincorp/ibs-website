...
export function ConsultationForm({ onSuccess, onSubmittingChange }: { onSuccess?: () => void; onSubmittingChange?: (submitting: boolean) => void }) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const fields: Record<string, string> = {
      ...
      phone: data.phone,
      email: data.email,
      'cf_location': data.location,
      'cf_loan_or_referral': data.loanOrReferral,
      'cf_loan_requirement': data.loanRequirements,
      'cf_loan_type': data.loanType,
      'cf_cibil_score': data.cibilScore,
